...
export const MobileBenefits: React.FC = () => {
  const [beneTab, setBeneTab] = useState<BeneTabType>('intro');

  return (
    <div className={styles.mobileBeneSection}>
      {/* 상단 타이틀 */}
      <div 
        className={styles.mobileBeneTitleArea}
        onClick={() => setBeneTab('intro')}
      >
        <span>To Our Customer</span>
      </div>

      <div className={styles.mobileBeneDashedDivider} />

      {/* 3열 탭 그리드 */}
      <div className={styles.mobileBeneGrid}>
        {BENE_ITEMS.map((item) => {
          ...
          return (
            <button
              key={item.id}
              type="button"
              className={`${styles.mobileBeneTabBtn} ${isActive ? styles.activeTab : ''}`}
              onClick={() => setBeneTab(item.id)}
            >
              <span className={styles.mobileBeneNum}>{item.num}</span>
              <span className={styles.mobileBeneLabel}>{item.title}</span>
            </button>
          );
        })}
      </div>
      ...
      {/* 하단 세부 내용 카드 */}
      <div className={styles.mobileBeneCard}>
        {beneTab === 'intro' ? (
